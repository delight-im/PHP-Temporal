...
{
    /** @var \DateTimeImmutable the underlying representation of date and time */
    private $dateTime;
    ...
    {
        $instance = new self($this->dateTime->$dateTimeInstanceMethodName(new \DateInterval($duration)));

        // if the *actual* day does not exist in the *intended* month
        // which causes PHP to calculate *wrongly*
        // we fix the calculation retroactively
        if (Iso8601Duration::affectsMonthsOnly($duration) && $instance->getDay() !== $this->getDay()) {
            return $instance->withModification('last day of previous month');
        }
    ...
    public static function now($timeZone = null)
    {
        ...
        return new self(new \DateTimeImmutable(null, self::makeTimeZone($timeZone)));
    }
    ...
    {
        try {
            return new \DateTimeZone($identifier ?: \date_default_timezone_get());
        } catch (\Exception $e) {
    ...
    {
        $parsed = \DateTimeImmutable::createFromFormat($format, $dateTime, self::makeTimeZone($timeZone));
        if ($parsed !== false) {
    ...
    {
        return self::createInstanceFromFormat(Iso8601Date::FORMAT_EXTENDED, $date, $timeZone);
    }
    ...
    {
        return self::createInstanceFromFormat(Iso8601Date::FORMAT_BASIC, $date, $timeZone);
    }
    ...
    {
        return $this->setTimeZoneInstance(self::makeTimeZone($identifier));
    }
